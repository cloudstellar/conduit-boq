...
function createDocumentInfoSheet(workbook: Workbook, dataset: CatalogExportDataset) {
  ...
  const rows: Array<[string, string]> = [
    ...
    ['วันที่มีผลบังคับใช้', displayDateWithIso(dataset.version.effectiveDate)],
    ['เลขที่อ้างอิงการอนุมัติ', dataset.version.approvalReference ?? '-'],
    ['วันที่เอกสารอนุมัติ', displayDateWithIso(dataset.version.approvalDocumentDate)],
    ['เผยแพร่เมื่อ / โดย', displayPublishedStamp(dataset)],
    ['ส่งออกเมื่อ / โดย', `${formatThaiDateTime(dataset.exportedAtIso)} / ${dataset.exportedBy.displayName}`],
    ['ฉบับตั้งต้น', dataset.version.basedOnVersionString ?? '-'],
    ['จำนวนรายการภายใต้ค่าแฮชชุดข้อมูล', dataset.counts.rowCount.toLocaleString('th-TH')],
    ['รายการใช้งาน / ยกเลิกใช้', `${dataset.counts.activeRows.toLocaleString('th-TH')} / ${dataset.counts.inactiveRows.toLocaleString('th-TH')}`],
    ['ค่าแฮชชุดข้อมูล SHA-256', dataset.canonicalDatasetHash],
    ['วิธีตรวจสอบ', 'นำค่า _canonical_row_json ในชีต "ข้อมูลตรวจสอบ" ตามลำดับมาต่อเป็น UTF-8 "[" + JSON คั่นด้วย "," + "]\\n" แล้วคำนวณ SHA-256 ให้ตรงกับค่าแฮชชุดข้อมูลด้านบน'],
    ['รุ่นข้อกำหนดการจัดรูปแบบข้อมูล', CATALOG_CANONICALIZATION_REVISION],
    ['รุ่นข้อกำหนดการส่งออก', CATALOG_EXPORT_SPEC_REVISION],
    ['สร้างโดย', CATALOG_EXPORT_APP_NAME],
    ['ขอบเขตข้อมูล Factor F / BOQ', 'ไม่รวม Factor F หรือ BOQ ในชุดข้อมูล ค่าแฮช จำนวนรายการ และ workbook นี้'],
    ['หมายเหตุ Binary file hash', 'คำนวณ Binary SHA-256 หลังบันทึกหรือจัดเก็บไฟล์ และบันทึกไว้นอก workbook นี้'],
  ];
  ...
  if (!dataset.isOfficialPublishedExport) {
    sheet.getCell(rowIndex, 1).value = 'ค่าแฮชชุดข้อมูลฉบับร่าง';
    sheet.getCell(rowIndex, 2).value = 'ค่าแฮชชุดข้อมูลฉบับร่างไม่ใช่ค่าแฮชสำหรับการเผยแพร่อย่างเป็นทางการ';
...
function createChangeSummarySheet(workbook: Workbook, dataset: CatalogExportDataset) {
  ...
  const metadataRows: Array<[string, string]> = [
    ...
    ['หมายเหตุประวัติ', 'ประวัติรายแถวฉบับเต็มอยู่ในระบบ ไม่รวม JSON ก่อน/หลัง และเนื้อหา workbook ที่ถูกปฏิเสธไว้ในไฟล์นี้'],
  ];
  ...
  if (dataset.changeSummaryScope === 'limited') {
    sheet.getCell(rowIndex, 1).value = 'จำกัด';
    sheet.getCell(rowIndex, 2).value =
      'ประวัติชุดการเปลี่ยนแปลงโดยละเอียดดูได้สำหรับผู้ดูแลที่มีสิทธิ์ในระบบ';
    sheet.mergeCells(rowIndex, 2, rowIndex, 12);
    styleDataRow(sheet, rowIndex, 12);
  } else if (dataset.changeSets.length === 0) {
    sheet.getCell(rowIndex, 1).value = 'ไม่มี';
    sheet.getCell(rowIndex, 2).value = 'ไม่พบชุดการเปลี่ยนแปลงที่อยู่ในขอบเขตของฉบับบัญชีราคา';
    sheet.mergeCells(rowIndex, 2, rowIndex, 12);
    styleDataRow(sheet, rowIndex, 12);
  } else {
    for (const changeSet of dataset.changeSets) {
      const values = [
        displayChangeTypeThai(changeSet.changeType),
...
function displayStatusThai(dataset: CatalogExportDataset): string {
  if (dataset.version.status === 'draft') return 'ฉบับร่าง';
  return dataset.version.status === 'active' ? 'เผยแพร่แล้ว' : 'จัดเก็บถาวร';
}

function displayChangeTypeThai(changeType: string): string {
  switch (changeType) {
    case 'clone':
      return 'สร้างฉบับร่างจากฉบับตั้งต้น';
    case 'import':
      return 'นำเข้า';
    case 'manual':
      return 'แก้ไขด้วยตนเอง';
    case 'publish':
      return 'เผยแพร่';
    case 'restore':
      return 'กู้คืนฉบับ';
