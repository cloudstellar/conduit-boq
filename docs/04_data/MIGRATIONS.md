...
| `023_master_catalog_phase4_published_code_rls_scope.sql` | P-39R forward-only RLS correction: active staff may read a code only when the exact `(identity_id, item_code)` pair occurs in an active/archived issued snapshot; active admins retain complete registry/history access | **Corrected Local-only source candidate SHA-256 `cbe01f63c6dd822edb29e1f7a31bfd27d5cb063e4d7d7e3878567875434d0a88`; first apply from `072294d` failed only its textual policy postcondition and rolled back completely; corrected apply and full P39R-L rerun pending; not Production-applied** |
| `017+_master_catalog_phase4_*.sql` | Umbrella reference for the Local-only Phase 4 range; the canonical bootstrap source now applies `017`-`023` after hotfix `016` | **Local-only range — P-36 clean-integrated execution through `021` and pre-P-39R evidence remain historical; `022` incremental invariants passed, `023` source/static correction is pending live proof, and P39R-L/P39R-C/P39R-U remain open before P-38 resumes; every Production approval remains absent** |
...
identity/code counts instead of assuming a clean 710-row registry. P-38 remains
paused until the complete P-39R gates pass.
...
local-only Phase 4 scripts `017` through `023`, and runs the bootstrap smoke
