...
| `016_hotfix_preserve_boq_item_suffix.sql` | Redeploy `save_boq_with_routes` to preserve approved BOQ item suffix labels while keeping catalog-backed unit, price, category, and version checks authoritative | **Production hotfix planned before Master Catalog Phase 4** |
| `017+_master_catalog_phase4_*.sql` | Master Catalog Phase 4 database migrations | **Planned — must be rebased/renumbered after hotfix 016 lands on `main`** |
...
applies `011`, Factor F `012` through `015`, hotfix `016`, and runs the Phase 2
smoke tests.
