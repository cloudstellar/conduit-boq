...
| T-58 | A short-code assumption rejects a valid full authority category key, or a disabled retirement capability turns a complete Full import review into an opaque failure/partial preview | Treat `categoryCode` as an exact versioned dictionary key/full label; shared 500-character bound; preflight compares the live maximum (89 currently); retirement-disabled validation returns the complete authoritative read-only diff with no persistence/change set and no Apply control | Boundary tests accept the real 89-character key and reject over 500; preflight fails on dictionary/contract drift; full source/E-01/E-02 preview proves counts and zero-write/no-Apply behavior | Exact pushed P-41 source and live smoke passed; Medium until fresh scored Card C |
