...
| T-55 | Legacy/extra RLS or DML policies, a service credential used as a surrogate admin, or incomplete terminal metadata allow role/state leakage, unattributed commands, direct writes, timestamp drift, or partially published rows | Explicit active-staff published/archived reads, active-admin draft/audit reads, exact one-policy-per-table allowlist, obsolete DML policy removal, no `service_role` execute on user-command RPCs, terminal no-op immutability, preserved backfill timestamps, complete publication constraints | Exact policy inventory and unexpected-policy denial, staff/admin/inactive/anonymous matrix, authenticated/service-role RPC grant matrix, direct-DML denial, backfill fixture, incomplete publication denial, terminal no-op update denial | Medium until P39R-L/P39R-C pass |
