...
| T-49 | The server-side placement review is accepted, but the admin changes a local assignment while a page-level banner still says accepted, then leaves believing the change was saved | Derive visible state from server acceptance plus client dirty comparison; replace every accepted claim with **ยังไม่ยืนยัน** while dirty; preserve pending choices across paging/supported return; warn or provide explicit recoverable continuation before leave/reload | Component/state tests plus desktop/mobile keyboard walkthrough: edit an accepted assignment, verify no contradictory banner, exercise leave/reload continuation, confirm no DB mutation before batch submit | P-34 source/static controls passed on `0780925`; Medium until integrated WP-8/P-14 browser/accessibility/UAT evidence passes |
