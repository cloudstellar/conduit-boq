...
| T-53 | A safe-error workbook changes a mapped price/name that the parser replaces with Production authority, so the claimed error never occurs; a generated `.xlsx` renders in one tool but cannot be opened by the application's ExcelJS parser; or a derivative and its self-declared manifest change together and still appear internally consistent | Design fixtures against the actual parser trust boundary; use an unmapped Local candidate for E-01 and exact mapped omissions for E-02; mark every derivative non-authoritative; pin the approved manifest/source/authority/binary hashes in tracked preflight code; compatibility-save and verify final binaries with the application ExcelJS dependency before Owner use | `db:local:p38:verify-inputs` first rejects any non-pinned manifest or input hash, then compares every required field, 708/708/693 rows, one E-01 code substitution, 15 E-02 mapped omissions, 710/65/17 frozen authority, and markers; visual QA covers all source-preserved sheets and rerenders changed final sheets | Low for the prepared P-38 inputs; rerun after any parser, authority, source workbook, or fixture change |
