...
| T-63 | A fallible executor self-authors or alters the schema contract, impersonates the independent reviewer with free-form text, or relies on a stale/dismissed review | Contract v2 removes free-form reviewer claims and hash-binds a structured immutable GitHub PR-review envelope to exact repository/PR/review ID/login/APPROVED state/source HEAD/pass-1 payload/time. A distinct human checks the review in an authenticated GitHub session before contract freeze and immediately before GO; approval v2 records the post-pass-2 `githubReviewCheckedAt`. The offline runner checks canonical structure, identity equality, hashes, and chronology but does not claim GitHub authentication or signature proof. | Valid/malformed URL, wrong repository/ID/login/type/state/commit, payload/marker tamper, legacy-field, chronology, reviewer/executor separation, and approval recheck-time tests; manual pre-freeze/pre-GO checklist | Medium residual accepted under the honest-but-fallible model: account compromise, deliberate fabrication/collusion, and post-check deletion remain outside current scope. If malicious-operator resistance or non-repudiation is required, stop and introduce signed attestations with independent key custody before Production; no custom PKI is added now |
