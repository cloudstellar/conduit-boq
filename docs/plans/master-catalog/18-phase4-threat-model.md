...
| T-50 | A large Supplement batch or 710-row context causes lag, focus loss, or mechanical per-row review, leading the admin to rubber-stamp placement or miss incomplete/conflicting rows | System suggestions plus explicit suggested/admin-modified/incomplete/invalid states; direct review-needed filter; bounded paging/search; complete impact summary; keyboard-complete non-drag path; measure agreed realistic batch against 710 inherited rows and keep arbitrary inherited reorder out of V1 | Named browser/device measurements for render/search/anchor/preview/paging/move/confirm plus independent intended-admin UAT explaining shifted-row impact without reviewing inherited rows individually | P-36 route/deep-page/export measurements and owner placement perception passed; Medium until Closure Matrix C-11 names import-preview/publish-readiness interaction baselines and C-07 completes UI confirmation |
...
| T-52 | Split technical and owner evidence is summarized as one complete end-to-end UAT, or current status says only owner approval remains even though the governing exit gate still requires a UI submission, broader core-admin flow, safe errors, or named performance evidence | One gate-to-evidence closure matrix plus the scored [Owner UAT Script #35](./35-phase4-wp8-p37-evidence-reconciliation-and-owner-uat-script.md); distinguish technical RPC/readback, developer preflight, owner interaction, and package acceptance; executable authority checks reject the overbroad conclusion; all open rows require exact evidence or explicit risk metadata | Consistency test requires Notes #34-#36, P-38 boundary, final owner UI submission status, E-01/E-02/E-03, broader independent UAT, and named import/readiness performance rows; review current Tracker/Verification/Decision/Migration summaries together | Medium until Closure Matrix C-07 through C-11 pass and the C-12 guard remains green; P-37 remains HOLD |
