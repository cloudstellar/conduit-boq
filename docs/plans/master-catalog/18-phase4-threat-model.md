...
| T-27 | Clean environments contain the same business catalog but produce unrelated hashes because baseline identities are random | P-20 defines deterministic baseline identities or an explicit dual-hash model; migration/spec/export/tests change together; do not claim equivalence before closure | Two clean approved environments/rebuilds reproduce the selected hash contract and identity mapping | Medium until P-20/WP-6.5 closes |
...
| T-29 | An admin completes a draft/import and learns only at publish that placement or retirement policy blocks it | Mirror DB blockers in draft/import preview and publish readiness with Thai remediation; DB remains final invariant | UAT adds/new identities and retires rows, sees blocker before apply/publish, preserves reviewable draft | Medium until P-18/P-19 workflow gates close |
