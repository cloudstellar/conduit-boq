...
**Current P-37 residual-risk decision (2026-07-25):** the Owner explicitly
accepts the disclosed guided-UAT evidence for WP-8/P-37 against exact
`df44b827b290933463da5e14fa9125314660022a`. The Owner requested the
button-by-button guidance; evidence remains labelled guided rather than
independent/no-assistance. This accepts only the P-37 human-workflow residual
risk. Technical database, authorization, stale-lock, idempotency, audit,
readiness, and cleanup controls were not waived. Add/Supplement remains hidden
until P-14, and P-12/Production remain separate threat reviews.
...
P-33 accepted the bounded technical checkpoint at 2026-07-15 13:54 +07.
P-34 exact application checkpoint `0780925` then implemented the source-side
T-49/T-50 dirty-state, recovery, exception-filter, keyboard, impact, and list-
bounding controls. Integrated Local browser/accessibility/performance evidence
and independent UAT remain separate.
...
**P-39R draft-identity/release-number amendment:** 2026-07-18 — owner review of
P-38 Card A found a second T-46 failure mode: permanent reservation makes an
internal abandoned attempt look like a missing official release. P-39 separates
immutable draft references from claimed/issued target versions. Abandonment
retains the complete attempt and releases its unissued target; publication
issues the target permanently. P-39R additionally closes global draft-race,
stale-abandon, restore-effect audit, terminal lifecycle, publication-metadata,
and legacy-policy drift risks. Incremental `022`/`023` invariants passed; the
continued live gate exposed row-trigger amplification on a 710-row clone, now
tracked as T-56 and owned by bounded forward `024`. P39R-S, incremental P39R-L,
and separately approved clean-chain P39R-C passed through exact pushed
`10531610eac53a97c6ef8f9d06418766b58bee36`. P39R-U remains required before
P-38 resumes. Production remains unauthorized.

The restore-effect, state-scoped RLS/lifecycle, and bulk-trigger controls are
tracked as T-54 through T-56 so the previously accepted T-48 through T-53
identifiers remain stable.

**P-41 discovery amendment:** 2026-07-19 — T-58 and T-59 cover two newly
observed boundary failures: application constraints/read-only behavior drifting
from live versioned authority, and draft-only withdrawal leaving a gap that a
client preview masks until the database rejects placement. The bounded
controls are a shared 500-character category-key contract plus live maximum
preflight, a complete non-persistent/no-Apply preview when retirement is
disabled, client gap rejection, and forward-only migration `025` for atomic
relative-order-preserving compaction with one placement-revision advance.
Migration `025` SHA-256 is
`00d79d7750aa52ba7f003f6bb82fedb1d31ab111be417d74329c1cd3d899f76f`.
Incremental Local apply, exact pushed source
`bb27b0d28e116e97ce1e7ee3e582f39bcc4edf22`, full source checks, and live
WP-6.6 smoke passed. Owner-approved exact execution source
`adcca3939f3080cdf64bc6ad807051e9e85fed94` then passed the clean
`009`-`015`, hotfix `016`, `017`-`025` chain, all four integrated smoke suites,
canonical verification, and final disabled-baseline readback. Scored-UAT
evidence remains open. Production remains unauthorized.

**P-46/P-47 callability amendment:** 2026-07-29 — T-64 records the fail-closed
P-46 finding that an authenticated `SECURITY INVOKER` wrapper can be correctly
granted yet fail on a guarded branch when its pure private formatter remains
owner-only. P-47 authorizes repository-only append-only migration
`026_master_catalog_phase4_catalog_action_error_acl.sql`, SHA-256
`472fa04b81bc8e96e9b507e20fc20cfee3114c80fda45f2ffba3893480920d8a`, to
make that one helper invoker and grant execute only to owner plus
`authenticated`, while preserving the `017a` default-privilege guard and
denying `PUBLIC`, `anon`, and `service_role`. This is a bounded availability
and stable-error-path repair, not a privilege expansion to data or a substitute
for the pre-`018` bridge. Local execution and all Production gates remain
unauthorized.
...
| T-37 | A partial 20-row view or hidden automatic draft choice causes the admin to miss an item, edit the wrong draft, or discover stale base only after submit | Full-catalog read within measured threshold; database-enforced one mutable draft globally; exact item/draft selection; stale draft permits only audited abandon and abandoned history is read-only; server verifies exact version/base/lock | Browser/UAT search first/middle/last rows, prove one global workspace, and prove stale mutation/import/placement/readiness/publish controls are disabled before submit | Historical G1R/G3 evidence retained; Low only after P39R and WP-8 independent UAT |
...
| T-43 | Two drafts from the same or different bases race, or an admin deletes/repurposes a draft to start over, splitting intent and losing trustworthy lineage | Global partial unique draft index; pointer/base locks; stable `DRAFT_ALREADY_EXISTS`; audited idempotent current/stale `draft -> abandoned`; no row/audit deletion; abandoned rows immutable even on no-op updates | Same-request replay, different-request duplicate, same-base and cross-base two-session create, current/stale valid/invalid/replayed/concurrent abandon, role denial, rollback, and zero-partial-effect tests | Incremental P39R-L, clean P39R-C, and owner P39R-U passed; historical G1R/G2/G3 evidence retained; Low pending broader WP-8 UAT |
...
| T-46 | The UI assumes revision, an admin guesses raw segments, a stale registry proposes a duplicate/skipped number, or one identifier conflates an abandoned work attempt with an official release and creates unexplained gaps | Required business intent/year; immutable `{target}-D{nnn}` draft reference backed by separate target/attempt columns; complete issued/currently-claimed registry or fail closed; target claim while mutable; atomic issue on publish; release on audited abandon; DB next-lane/concurrency/replay guards; published/archived tuples immutable and never reusable | All-lane unit fixtures; incomplete-registry denial; migration/grant/lifecycle checks; live create race/replay, abandon/replay, null official tuple with retained target/snapshot/audit, replacement new draft reference plus same target, publish issue, clean bootstrap, and owner Card A | P39R-S, incremental P39R-L, clean P39R-C, and owner P39R-U passed; historical G1R/G2 permanent-reservation evidence retained; Low |
| T-54 | Restore changes the current pointer while an open draft exists, but the operator/audit cannot tell whether that draft remains current or became stale | Confirmation computes the effect; DB transaction records pointer before/after, affected draft, and constrained effect; restore replay returns the same audit identity/effect | Current/no-draft, remains-current, becomes-stale, replay, race, and rollback tests | P39R-L and clean P39R-C passed; Low pending broader WP-8 use |
| T-55 | Legacy/extra RLS or DML policies, identity-only code visibility, a service credential used as a surrogate admin, or incomplete terminal metadata allow draft/state leakage, unattributed commands, direct writes, timestamp drift, or partially published rows | Explicit active-staff published/archived reads; code visibility requires the exact `(identity_id, item_code)` pair used in an issued snapshot; active-admin draft/audit reads; exact one-policy-per-table allowlist; obsolete DML policy removal; no `service_role` execute on user-command RPCs; terminal no-op immutability; preserved backfill timestamps; complete publication constraints | Exact policy inventory and unexpected-policy denial, accumulated published-history counts, transient draft-only code denial, staff/admin/inactive/anonymous matrix, authenticated/service-role RPC grant matrix, direct-DML denial, backfill fixture, incomplete publication denial, terminal no-op update denial | `023`, incremental P39R-L, and clean P39R-C passed; Low pending P-12 minimization review |
| T-56 | A row-level placement trigger turns a 710-row clone or high-volume import into repeated whole-draft anti-joins, hits the RPC statement timeout, and leaves the admin unable to create or update a draft | Forward `024` uses one transition-table calculation per SQL statement, direct set-based new-identity detection, transaction-local invalidated/no-candidate version markers, and the existing placement-RPC bypass; no permanent cache state or duplicate apply RPC | Exact trigger-inventory/least-privilege postconditions; full WP-6.6 clone/import and WP-7.5 add/review/stale-revision live suites; timeout rollback and final pointer/flag/BOQ/Factor invariants | `024` passed incremental P39R-L on exact `b6d58ce` and clean P39R-C on exact `1053161`; Low pending broader WP-8 scale evidence |
...
| T-49 | The server-side placement review is accepted, but the admin changes a local assignment while a page-level banner still says accepted, then leaves believing the change was saved; conversely, reset-all can leave a stale recovery claim after pending work is gone | Derive visible state from server acceptance plus client dirty comparison; replace every accepted claim with **ยังไม่ยืนยัน** while dirty; preserve pending choices across paging/supported return; warn or provide explicit recoverable continuation before leave/reload; show recovery copy only while pending browser-local work exists and clear its flag on reset-all | Component/state contracts plus browser walkthrough: edit an assignment, verify no contradictory banner, exercise leave/return/reload continuation, confirm no DB mutation before batch submit, then reset and verify 18/0 with the recovery alert gone | Recovery/reset and owner keyboard/focus checks passed through `f36d896`; Medium until Closure Matrix C-07 proves accepted UI state followed by a local dirty change in one owner session |
| T-50 | A large Supplement batch or 710-row context causes lag, focus loss, or mechanical per-row review, leading the admin to rubber-stamp placement or miss incomplete/conflicting rows | System suggestions plus explicit suggested/admin-modified/incomplete/invalid states; direct review-needed filter; bounded paging/search; complete impact summary; keyboard-complete non-drag path; measure agreed realistic batch against 710 inherited rows and keep arbitrary inherited reorder out of V1 | Named browser/device measurements for render/search/anchor/preview/paging/move/confirm plus intended-admin evidence explaining shifted-row impact without reviewing inherited rows individually; independent/no-assistance scoring remains the P-14 default unless separately varied | C-07/C-11 passed for WP-8 and the Owner accepted the disclosed guided-UAT residual; Medium accepted for WP-8 and must be re-evaluated before P-14 Add/Supplement enablement |
| T-51 | Closing a controlled editor, final-review, or guarded-leave dialog loses the keyboard user's place; separately implemented summaries drift and make the same final position look different across screens | Store only the exact transient invoking element, restore connected opener focus with `preventScroll`, use native controls, and reuse one responsive previous/new/next preview with DB-derived final position kept separate from authority-owned names | Focused regression contract covers all three dialogs and shared preview; owner keyboard re-UAT closes/reopens editor and final review with immediate `Enter`; production build/type/lint/full suite pass | Final corrective checkpoint `f36d896d672609653de6634e307dcc44bce6d519` passed; Low for the exercised V1 path |
| T-52 | Split technical and owner evidence is summarized as one complete end-to-end UAT, or a guided run is silently relabelled independent/no-assistance | One gate-to-evidence closure matrix plus [Owner UAT Script #35](./35-phase4-wp8-p37-evidence-reconciliation-and-owner-uat-script.md); distinguish technical RPC/readback, developer preflight, Owner-operated guided interaction, and package acceptance; any variance requires an explicit Owner decision and truthful evidence label | Consistency test requires Notes #34-#36, P-38 boundary, C-07/C-09 labels, E-01/E-02/E-03, named import/readiness performance rows, and the 2026-07-25 variance; review Tracker/Verification/Decision/Migration summaries together | Low for P-37 after C-07 through C-12 passed and the Owner accepted the disclosed guided-UAT residual; reopens before P-14 if the evidence label or relevant flow changes |
| T-53 | A safe-error workbook changes a mapped price/name that the parser replaces with Production authority, an approved `.xlsx` stores source rows/money as numeric cells the profile rejects, a file opens in ExcelJS but not the actual application parser, or a derivative and its self-declared manifest change together | Design against the actual trust boundary; use unmapped E-01 and exact E-02 omissions; mark derivatives non-authoritative; pin manifest/source/authority/binary hashes; accept only safe integer/numeric-money cells while rejecting formulas/dates/booleans/fractions/unsafe values; run the actual application adapter/profile before independent ExcelJS field comparison | `db:local:p38:verify-inputs` rejects changed hashes, executes adapter/profile over all 708/708/693 retained rows, compares one E-01 substitution, 15 E-02 omissions, 710/65/17 authority, fields, and markers; parser tests cover accepted numeric and rejected unsafe cell kinds | P-40 real-parser input verification and focused/full tests passed; Low, rerun after any parser, authority, source workbook, or fixture change |
| T-57 | Browser-native validation speaks English, unit free text drifts from catalog conventions, a successful withdrawal strands the admin on a missing-item error, or the UAT orders a stale-publish check after an independent structured-code guard makes it unreachable | Shared Thai money normalization and exact-cent sum; base-version unit combobox plus explicit custom path; fail closed if governed units cannot load; server-side post-withdraw redirect with safe return-path validation and durable Thai notice; perform E-03 in Card A before any structured-code add and never bypass the guard | Money/navigation unit tests; action/parser tests; TypeScript/lint/build; real-parser preflight; exact pushed `dc83c35`; developer browser QA plus the truthfully labelled Owner-guided Cards A/B/E evidence | P-40 source/docs and functional evidence passed; Medium residual accepted for WP-8 under the guided-UAT variance and due for independent/no-assistance revalidation before P-14 |
| T-58 | A short-code assumption rejects a valid full authority category key, or a disabled retirement capability turns a complete Full import review into an opaque failure/partial preview | Treat `categoryCode` as an exact versioned dictionary key/full label; shared 500-character bound; preflight compares the live maximum (96 currently); retirement-disabled validation returns the complete authoritative read-only diff with no persistence/change set and no Apply control | Boundary tests accept the real live key and reject over 500; preflight fails on dictionary/contract drift; full source/E-01/E-02 preview proves counts and zero-write/no-Apply behavior | Exact pushed P-41 source/live smoke and P-42 recovery preflight/Card C functional evidence passed; Medium until independent targeted Card C |
| T-59 | Withdrawing a never-published row deletes one numeric position, the client silently resequences the preview, and the DB later rejects placement or an implementer weakens the contiguous-order guard | Preserve the DB guard; client rejects preexisting gaps; forward-only `025` installs one AFTER DELETE statement trigger with OLD transition table, compacts only affected drafts under advisory lock/deferred uniqueness, preserves relative order, and reuses `024` transaction markers so revision advances once | Static trigger/grant/migration-order checks; live withdraw asserts exact `0..N-1`, relative order, exactly-one revision advance, stale/replay/rollback behavior, and no BOQ/Factor F effect; clean `017`-`025` chain | Incremental and clean-chain exact-source smoke passed the compaction/revision/adjacent-data assertions; Low technically, fresh scored Cards B/D remain for independent operator evidence |
| T-60 | An old final-review tab silently adopts a newer draft lock, or a successfully published version still displays draft/stale-base wording, causing the operator to submit an action they believe is stale or misread a successful irreversible publication as a rejection | Canonical draft-review URL binds `reviewLock`; mismatched lock is a hard read-only stop with old/current locks and no diff/publish panel; latest-review navigation is explicit; draft-only stale-base alert; published/abandoned historical labels; server/DB expected-lock and idempotency remain final authority | Pure binding tests; source contract proves publication requires current binding; real published route has no stale-base/draft action and uses read-only before/after labels; fresh Owner Card A returns to an old tab, sees no publication form, opens latest, and produces zero publish/pointer effect | P-42 Local incident preserved one current-lock publication and no stale-lock bypass; focused 3 files/26 tests, full 35 files/225 tests, TypeScript, lint, authority/input checks, network-enabled build, diff check, and published-state visual QA passed. The separately approved P-42 recovery Card A then passed the stale hard stop and zero publication/pointer effect functionally; Medium until independent targeted Card A confirms the same path without guidance |
| T-61 | An admin cannot identify a newly allocated code, cannot wheel-scroll a nested placement list, silently loses stale browser choices, mistakes a browser print view for a PDF download, or receives a generic error for an uncertain transport outcome | Read the allocated code/name from the immutable audited add effect; confine non-passive wheel handling to the open placement list; scope browser choices by version/lock/placement revision with an explicit stale-user-choice notice; label the export command as opening print/save-PDF; return `CATALOG_OUTCOME_UNCERTAIN` with the same request ID | Action-model/add-effect and placement-storage unit tests; operator-workflow copy assertions; focused and full repository gates; independent targeted Cards B/D/E/F confirm the rendered recovery paths | Exact bounded correction `bdc104f77f18ea8fc776950259bc25e68c2fd42a` passed focused 3 files/31 tests, full 36 files/229 tests, TypeScript, lint 0 errors/10 existing warnings, authority/input checks, network-enabled build, and clean Local readback. Medium until the affected cards are independently rerun; no migration, reset, or Production action |
| T-62 | A future `SECURITY DEFINER` or public RPC inherits PostgreSQL `PUBLIC EXECUTE`, or Supabase's schema-specific `service_role` default re-adds EXECUTE after a global revoke | Frozen forward bridge `017a` executes as/owns objects with `postgres`; revoke global and `public`/`private` API-role function defaults; global ACL owner-only and schema ACL owner-only/absent; normalize four `017` stubs; exact routine ACL/security/search-path/body inventory at every later stage; only the bridge may follow `017` | Static migration/hash/order tests; stage-aware default/routine ACL runner checks including `service_role`; fresh isolated pass 1, independently reviewed schema contract, pass 2, advisor hand-offs, and final closeout | P-12 later passed the corrected sequence and closeout; Low for the exact applied bridge. P-49 does not reopen that evidence |
| T-63 | A fallible executor self-authors or alters the schema contract, impersonates the independent reviewer with free-form text, or relies on a stale/dismissed review | Contract v3 removes free-form reviewer claims and hash-binds a structured immutable GitHub PR-review envelope to exact repository/PR/review ID/login/APPROVED state/source HEAD/pass-1 payload/time and the eleven-file candidate through `026`. A distinct human checks the review in an authenticated GitHub session before contract freeze and immediately before GO; approval v3 records the post-pass-2 `githubReviewCheckedAt`. The offline runner checks canonical structure, identity equality, hashes, and chronology but does not claim GitHub authentication or signature proof. | Valid/malformed URL, wrong repository/ID/login/type/state/commit, payload/marker tamper, legacy-field, chronology, reviewer/executor separation, and approval recheck-time tests; manual pre-freeze/pre-GO checklist | Medium residual accepted under the honest-but-fallible model: account compromise, deliberate fabrication/collusion, and post-check deletion remain outside current scope. If malicious-operator resistance or non-repudiation is required, stop and introduce signed attestations with independent key custody before Production; no custom PKI is added now |
| T-64 | A correctly authorized public `SECURITY INVOKER` wrapper reaches a validation, conflict, or guard branch but cannot execute its owner-only private result formatter, so a safe machine-coded response becomes `permission denied` and a workflow may leave an incomplete Local fixture | Keep privileged transactional helpers as definers, but classify the pure `catalog_action_error` formatter by behavior: exact reviewed body/signature/owner/empty search path, `SECURITY INVOKER`, and explicit owner-plus-`authenticated` execute only. Append `026` after `025`; preserve `017a` owner-only defaults; deny `PUBLIC`, `anon`, and `service_role`; make smoke cleanup best-effort/finally-safe | Static filename/ledger/hash/order/body/ACL tests; exact caller-graph and role matrix; guarded draft-create branch returns `DRAFT_ALREADY_EXISTS`; direct unauthorized calls fail; fresh separately approved canonical bootstrap and full WP-6.5/WP-7/final-invariant evidence | P-46 failed closed with no Production effect; the corrected `026` later passed P-12. Low for that exact callability contract |
| T-65 | UI, middleware, RLS, grants, RPC, settings, and privileged API status rules disagree, allowing a signed-in non-active identity to bypass the waiting UI, access business/profile-PII/settings data, self-promote `role`/`status`, pass a role-only helper, or reach a service-role operation | P-49 `pending = profile/onboarding-only` deny-by-default matrix; preserve active-only catalog RLS; forward-only BOQ/Factor-F/settings/profile SELECT+UPDATE grant and protected-column correction; exact self-scoped role helpers; every business/privileged server/RPC path requires current status plus approved role/scope; exact pending route/API allowlist; retained pending-owned data; no trust in stored role while status is non-active | Read-only live posture inventory; isolated real-session Data API/RPC matrix for anonymous/pending/active/inactive/suspended/missing/unknown; other-profile/settings/role-helper read and protected-column self-escalation negatives; non-active BOQ/catalog/legacy+versioned Factor-F denial; service-role API active-status tests; approval/rejection/resubmission/revocation E2E; unchanged approved active-user behavior; fresh CI/Preview | **Open / High; temporarily accepted and deferred under P-51 for the exact first closeout.** Source and frozen-baseline evidence establish credible privacy/authorization paths; exact current Production exploitability remains to be verified read-only under separate authority. P-49 implementation is not authorized, and P-51 does not remediate the risk |
...
- expected-error branches through every public invoker wrapper, including
  authenticated draft-create guard precedence and direct helper role denials;
...
- a requirement to resist a malicious executor/reviewer, provide
  non-repudiation, or operate without an authenticated GitHub review check.
