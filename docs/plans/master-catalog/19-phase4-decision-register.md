...
| P-42 final-review snapshot correction | Exact application/UAT/docs correction passed and pushed; clean Local recovery pending | Owner + developer | 2026-07-19 | Exact `b2500b5e6859a915bfa3f70d558934f252943f82`. Interrupted Card A issued Local `2568.5.0` through a current-lock request, then displayed a false draft-only stale-base warning. Review URLs now bind `reviewLock`, stale tabs hard-stop, and terminal review is read-only. No migration/Production/BOQ/Factor F/hotfix change; a new Local reset requires separate approval. |
