...
historical. G1R, independent G2, fresh browser/G3, G4, and every Production
action remain separate decisions.
...
| L-33 | Add/supplement rows may be created in draft, but publication of any version containing new identities is held until P-18 placement governance is approved or a DB-level publish guard rejects that path | Prevents a future admin from accidentally making append-at-end placement official |
...
| L-52 | A successful create opens the exact draft workspace; items/actions precede detailed document metadata; pointer restore requires current-to-target confirmation and explains that historical BOQs retain their saved version | Keeps the admin flow aligned with the release workspace and adds a human safety barrier around the separate recovery operation |
...
For P-08 through P-22, record the evidence in the Change Request, Runbook,
...
| WP-6.6 P-23.1 version/workspace correction | Bounded implementation authorized; repository/static passed; no reset approved | Owner | 2026-07-13 | P-23.1; amended `020` is unapplied and prior live evidence is historical; exact commit, G1R, independent G2, fresh browser/G3, G4, and all Production actions remain separate |
| WP-6.6 P-24 pre-G1R hardening | Bounded base implementation passed on `88d0711`; same-scope repeated-error-focus/provenance closure passed its working-tree gate; no reset approved | Owner | 2026-07-13 | Annual range, stable error, durable identical-retry focus, contextual authority, Factor F hierarchy, and exact execution-provenance contract; correction commit and G1R remain separate |
