...
- P-11 approved the field-facing export visual direction for implementation:
  ...
  version-effective-date, no truncated SHA-256 in the PDF footer, no technical
   ...
   embedded NT runtime fonts. Final P-11 acceptance still requires a replacement
   database-generated Excel/PDF pair after this typography refinement.
...
to TH Sarabun New with a 16-point body baseline. The technically verified
`edf3570a` pair is superseded by that font decision and was never filed or
accepted as final. WP-6 remains in progress until WP-6.5E produces one
replacement, semantically verified PDF/Excel pair whose binary hashes are
recorded before the owner's short final visual confirmation.

The owner authorizes WP-6.5 Local-only work to start while that final binary
acceptance remains pending. This authorization includes P-20 implementation
and the tracked verifier, but it does not authorize a Local Supabase reset
...
| Official export format | Direction including Excel TH Sarabun New 16 pt approved; replacement artifacts pending | Owner | 2026-07-11 | P-11; replacement DB-generated Excel/PDF must still pass count/hash, typography, accessibility, and filing checks |
