...
| L-34 | WP-6.5 also hardens the first structured-code publish boundary: active legacy `ITEM-####` rows in `2568.1.0` must be limited to the approved `ITEM-0139` exception | Prevents the P-06 exception from remaining only a reviewer checklist |
...
| P-18 | Decide display-order placement governance for newly added or supplement catalog items | Owner identified the core reason for Master Catalog governance during WP-6: future rows must be placed into their correct category/neighborhood instead of appearing at the end simply because they were added later. Recommended decision: keep append-only item codes and stable identities, keep draft add/supplement creation available for controlled review, and insert WP-6.5 to add a DB publish guard that rejects versions containing identities absent from the base version with `P18_PLACEMENT_REVIEW_REQUIRED`. The full placement/review workflow remains deferred until separately approved. | Owner + data custodian | Before Production enablement of add/supplement workflow and before P-15 for any version with added/supplement rows | Pending; WP-6.5 guard planned 2026-07-07 |
...
| P-20 | Decide the cross-environment canonical hash/identity portability contract | Current canonical hash includes stable `identity_id`, while Local migration `017` creates baseline identities with random UUIDs on each clean bootstrap. Recommended path: initialize each baseline stable identity deterministically from its existing immutable Production `price_list.id` (or another reviewed deterministic mapping) and keep `identity_id` in the lineage hash. Alternative: explicitly separate a business-content hash from a lineage hash. Do not silently remove identity from the approved hash or accept environment-specific hashes as equivalent without revising the parser/hash, DB, export, migration, and verification contracts together. | Owner + database/data-governance reviewer | Before WP-6.5 exit/WP-7, WP-8 clean rehearsal, migration fingerprint freeze, and P-15 hash acceptance | Pending; recorded 2026-07-11 |
