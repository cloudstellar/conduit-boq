...
runtime attempt failed closed with PostgreSQL `42704`; cleanup restored
`2568.0.0`, zero drafts, and all disabled flags. Fix-forward work may correct
that same placement defect and rerun Local evidence, but may not add `021` to
bootstrap or cross WP-8, P-19, Factor F/hotfix, feature, or Production gates.
...
| P-32 WP-7.5 Local live gate | Local reset/separate apply/live evidence authorized; first attempt failed closed and cleanup passed | Owner-authorized execution + developer evidence | 2026-07-15 | Error `42704` exposed an unqualified constraint under fixed search path; current same-scope fix candidate awaits replacement Local evidence. No Production action. |
