...
| P-42 final-review snapshot correction | Bounded application/UAT/docs correction directed; exact checkpoint and clean Local recovery pending | Owner + developer | 2026-07-19 | Interrupted Card A issued Local `2568.5.0` through a current-lock request, then displayed a false draft-only stale-base warning. Bind review URLs to `reviewLock`, hard-stop stale tabs, and render terminal review read-only. No migration/Production/BOQ/Factor F/hotfix change; a new Local reset requires separate approval. |
