...
*   **งานปฏิบัติ**:
    1.  รันคำสั่ง SQL นับและตรวจสอบใบงาน `boq` และรายการไอเทม `boq_items` ในปัจจุบัน
        ...
        *   ต้อง refresh `price_list` count ผ่าน authenticated Supabase SQL/MCP โดย baseline เอกสารล่าสุดคือ 710 rows: ฐานเดิม 682 + PN6 28 (`ITEM-0683` ถึง `ITEM-0710`)
        *   ต้องตรวจ `factor_reference` ทั้งตารางผ่าน authenticated Supabase SQL/MCP: จำนวนแถว, duplicate/null, ลำดับ `cost_million`, ค่า factor ต้องเป็นบวก และ full-table checksum ต้องตรง; ค่า 30M/40M เป็นเพียง smoke example ของเคสสุรินทร์ ไม่ใช่ gate ทั้งหมด
