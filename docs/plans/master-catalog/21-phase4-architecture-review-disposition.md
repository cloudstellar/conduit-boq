...
| `display_order` source is unclear | **Accepted** | Legacy order is the verified unique numeric suffix of `ITEM-####`; clones preserve it and new items append for draft allocation only. Physical/workbook row order is prohibited. P-18/WP-6.5 implements a publish guard before any add/supplement/new-identity version can become official; live Local evidence and the placement decision remain pending. |
