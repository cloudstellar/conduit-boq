...
| `display_order` source is unclear | **Accepted** | Legacy order is the verified unique numeric suffix of `ITEM-####`; clones preserve it and new items append for draft allocation only. Physical/workbook row order is prohibited. P-18 V1 is accepted; WP-7.5 source/static implements one reviewed new-identity batch while preserving inherited relative order. Add/Supplement and publication remain held until Local live evidence passes. |
