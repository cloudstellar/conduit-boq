...
> **Post-027/028 security contract:** Raw `app_settings` remains closed to
> anonymous/authenticated clients. Migration 028 exposes only the two-boolean
> Admin-gate projection through a public security-invoker wrapper; the private
> definer rechecks the current active Admin and malformed configuration fails
> closed. Every Server Action reloads the gate, and each mutation RPC remains
> the final active-Admin plus `private.catalog_admin_enabled()` authority.
> Migration 028 also stops before object creation if the exact post-027
> predicate, catalog function/policy fingerprints, 15-table RLS/direct-DML
> posture, raw-settings ACL, or private-schema usage boundary has drifted.
> Closing the flag denies each new mutation request; it does not cancel an
> already-running transaction, and the UI becomes read-only after refresh or
> revalidation.
