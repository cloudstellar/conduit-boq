...
preview/publish-readiness interaction baselines also remain open. WP-8 is In
progress; P-37 remains HOLD and Production remains unauthorized. See
...
now supplies retained Card B-E functional evidence. Only the four
post-correction Owner spot-checks and cleanup remain open. Before
P-39, the historical
...
evidence rather than strict independent Owner-scored closure. P-37 remains
HOLD for bounded finding disposition/correction and proportional rendered
revalidation.
...
post-correction spot-checks in Note #35 Section 1.2 and cleanup remain. No reset
is implied.
...
| 4C clean rehearsal | Local | Codex + owner/developer | 2026-07-15 |  | P-36 integrated rehearsal and named P-37 technical/recovery/owner keyboard gates passed; WP-8 remains In progress and P-37 remains HOLD for the open Closure Matrix #34 rows | Exact checkout `910cc3c` passed the integrated chain through `021`, DB/RLS/concurrency/P-20/WP-7/WP-7.5, export/advisors/repository gates, realistic scale, and disabled-baseline cleanup. Later no-reset continuations passed stale/replay/accepted-state/recovery plus complete owner `Tab`/`Shift+Tab`, `Enter`/`Space`, sibling-order, focus-return, and final-presentation review. The owner did not submit the final placement batch through the UI; broader independent UAT and named performance/error-recovery rows remain open. Final source checkpoint `f36d896d672609653de6634e307dcc44bce6d519` is pushed; fixture `2568.15.0` was audited-abandoned and baseline invariants returned exact. |
| WP-8 P-42 bounded findings | Repository + read-only Local | Codex | 2026-07-20 | 2026-07-20 | Source correction passed; four post-correction Owner spot-checks and cleanup pending | Evidence checkpoint `1c901855a32b100013fb5c9472c2e909e3dd1c59` and bounded source checkpoint `bdc104f77f18ea8fc776950259bc25e68c2fd42a`; focused 3 files/31 tests, full 36 files/229 tests, TypeScript, lint 0 errors/10 existing warnings, authority/input checks, network-enabled build, and clean Local status passed. Completed functional Card B-E evidence is retained. No migration, reset, or Production action. |
...
| P-42 bounded finding correction | Owner + Codex | Source/procedure correction passed; four correction spot-checks and P-37 remain HOLD | 2026-07-20 | Exact evidence `1c901855a32b100013fb5c9472c2e909e3dd1c59`; exact bounded source `bdc104f77f18ea8fc776950259bc25e68c2fd42a`; focused 3 files/31 tests, full 36 files/229 tests, TypeScript, lint 0 errors/10 existing warnings, authority/input checks, network-enabled build, and clean read-only Local status passed. Completed Card B-E evidence is retained; only Note #35 Section 1.2 and cleanup remain. No migration, reset, or Production action. |
...
`private.catalog_placement_state`. Preserve accepted migration `021`; remove
the dead assignment only through a future reviewed forward migration/source
checkpoint before P-12, with affected fingerprints and harnesses rerun.
...
`f36d896d672609653de6634e307dcc44bce6d519`. P-37 remains **HOLD** only pending
...
|---|---|---|---|
| WP-8 clean Local rehearsal | Passed with no unresolved blocker | P-36 integrated technical rehearsal and named P-37 recovery/owner keyboard/focus/presentation checks passed; `2568.15.0` cleanup returned the exact disabled baseline. Final owner UI submission and broader independent WP-8 UAT/performance closure remain open. | HOLD under Closure Matrix #34 |
| WP-6.6 capability matrix | Audit #29 C-01 through C-17 implemented/evidenced, or unsupported controls removed from release visibility | Final G1R/G2/P-25/G3/P-26 path was owner-accepted at `78e96ab`; P-36 integrated rerun passed. Those technical/operator checkpoints do not silently satisfy the broader independent WP-8 UAT. | Accepted/Complete for WP-6.6; independent WP-8 closure remains separate |
| Reviewed migration fingerprint | Filename and SHA-256 match approved file | P-36 applied `021_master_catalog_phase4_placement_governance.sql` SHA-256 `e4de258756bbfbda0508e55d7b76ba2e907f644625b49bc29d4a4d7ac42fa714` after `020` | Passed Local; Production fingerprint freeze remains P-12 |
| Repository/deployment fingerprint | Exact branch, commit, CI, and deploy artifact recorded | P-36 Local execution used exact branch `codex/master-catalog-phase4` checkout `910cc3cc74660beecf18655d39cd0b0c085d1fc6`; no deployment artifact exists yet | Local evidence passed; deploy fingerprint remains P-13 |
...
| Hotfix `016` / migration order | Remote ledger includes `016`; clean Local bootstrap applies `009`-`015`, hotfix `016`, then reviewed Phase 4 migrations before WP-8 evidence is accepted | P-36 clean-bootstrapped the integrated `009`-`021` authority path on exact `910cc3c`; fresh Production ledger verification remains a later approved read-only P-12 gate. | P-36 Local order passed; Production check pending |
| End-to-end request idempotency | UI/action/DB reuse one operation ID after timeout; changed payload with same ID rejects | Prior DB/transport/browser recovery passed; P-36 repeated replay/mismatch/race contracts in the integrated harnesses; P-37 repeated stale rejection and exact replay | Passed Local technical/recovery scope; independent owner recovery evidence remains under Closure Matrix #34 C-08/C-10 |
...
| Admin UAT and recovery | Intended admin completes core workflow and representative failures without developer/SQL assistance | Placement overview/gap/sibling/leave-reload/keyboard/focus/presentation checks passed across bounded sessions. The owner did not submit the final UI batch, the stale response was proved separately, and no current record closes broader core-admin UAT plus three named safe errors in one independent session. | Open: Closure Matrix #34 C-07 through C-10 |
| 710-row performance baseline | Import preview, readiness, export, and admin interactions meet reviewed budget | P-36 recorded 710+18 route render/deep paging/export measurements; corrected P-37 search, preview, sibling, final review, recovery, and focus had no material stutter. Named import-preview and publish-readiness interaction baselines remain incomplete. | Partial: Closure Matrix #34 C-11 |
| Authority/document consistency | Migration/WP order, decision IDs, authority links, current status, and Markdown table shapes agree | P-36 evidence remains valid. The P-37 closure alignment adds Note #34 and executable assertions against the overbroad only-owner-decision conclusion. Focused authority checks passed 1 file/7 tests; the full suite passed 33 files/184 tests; TypeScript, lint with 0 errors/10 existing warnings, authority 710/65/17, and diff check passed. | Passed; Closure Matrix #34 C-12 closed |
...
| Advisors | No unresolved Phase 4 blocker | P-36 security advisor returned zero issues. Performance advisor findings were triaged as baseline information; both `020` authority FKs remain indexed. One low-risk unused local assignment in accepted `021` is recorded for a future reviewed forward correction, not a migration rewrite. | Passed Local blocker gate; readiness-debt disposition remains before P-12 |
| Feature flag | Disabled by default before migration/deploy | Final P-37 cleanup retained all three catalog flags `false` | Passed Local baseline; deploy/enablement remain P-13/P-14 |
...
| Intended-admin UAT without developer/SQL help | Intended admin must distinguish system-arranged, locally adjusted, incomplete/invalid, unconfirmed, and accepted states; filter when needed; explain shifted sequence numbers; recover stale placement and leave/reload state; reach final review without irreversible error | P39R-U lifecycle behavior passed. Exploratory Cards B-F passed Card F and found UAT-01 through UAT-05, but live collaboration makes them discovery rather than scored evidence. P-40 corrections passed source checks and separate assisted developer browser QA only. The first scored P-42 Card A is invalid after successful Local publication; its unbound-review and false-terminal-state findings are corrected in source and the safer script no longer submits a known-stale request. | Partial: fresh scored C-07 through C-10 evidence remains open after exact P-42 and clean recovery |
| 710-row interaction/import/export performance | Measure 710 inherited rows plus an agreed realistic new-item batch for render, search, insertion-gap selection, preview recalculation, paging, sibling movement, confirmation, import, and export on a named browser/device | P-36 measured server navigation/render for 710 inherited + 18 new rows at 607-1,136 ms, deep 100-row paging at 746 ms, and independently verified baseline export. Corrected P-37 sessions exercised search, preview, sibling movement, final review, recovery, and focus at the same scale without material stutter or layout failure. | Partial: named import-preview and publish-readiness interaction baselines remain C-11 |
...
|---|---|---|---|
| `npm test` | Exit 0 | P-36 exact checkout `910cc3c`: 33 files/183 tests passed. Final P-37 source checkpoint `f36d896d672609653de6634e307dcc44bce6d519`: 33 files/184 tests passed, including insertion-gap and focus/presentation contracts. | Passed pushed source checkpoint |
| `npx tsc --noEmit --pretty false` | Exit 0 | P-36 exact checkout and final P-37 source checkpoint `f36d896` passed. | Passed pushed source checkpoint |
| `npm run lint` | Exit 0 | Final P-37 source checkpoint `f36d896` exited 0 with the same 10 existing application warnings outside this scope. | Passed with existing warnings |
| `npm run build` | Exit 0 | Final P-37 source checkpoint `f36d896` compiled, typechecked, generated all routes including placement, and retained only the existing middleware-to-proxy deprecation warning. | Passed with existing warning |
...
| `git diff --check` | Exit 0 | Passed before committing final P-37 source checkpoint `f36d896` after code/test alignment. | Passed pushed source checkpoint |
...
| Documentation consistency | Authority links/table shapes, migration order, WP order, decisions, and execution provenance agree | Exact recovery evidence checkpoint `1c901855a32b100013fb5c9472c2e909e3dd1c59` and exact bounded source checkpoint `bdc104f77f18ea8fc776950259bc25e68c2fd42a` are reflected across Architecture, Verification, Threat Model, Decision Register, Tracker, and Closure Matrix. The correction passed focused 3 files/31 tests, full 36 files/229 tests, TypeScript, lint with 0 errors/10 existing warnings, authority 710/65/17 SHA-256 `28675e6244c65d485dda7142634b381db729a139bccdf189ad51563251a2e12a`, input verification 708/708/693, network-enabled build, clean read-only Local status, and diff checks. | Passed for current checkpoint; update again after the four Owner spot-checks |
...
| P-41 category/preview/withdraw-order correction | Authority-key bounds, read-only preview semantics, gap rejection, post-withdraw compaction, and one-revision transaction semantics agree across app, migration, bootstrap, smoke, tests, and authority docs | Migration `025` SHA-256 `00d79d7750aa52ba7f003f6bb82fedb1d31ab111be417d74329c1cd3d899f76f`; exact source `bb27b0d28e116e97ce1e7ee3e582f39bcc4edf22`; clean execution `adcca3939f3080cdf64bc6ad807051e9e85fed94`; WP-6.5 `4b69e44dde915ca25c3f78379a1c45b002b31cb8aebcbf361ec3b58670f9e245`, WP-6.6 `e9e28eb1bb6f312a4638c0d67b00cb420864d5433295ffb80a95a12ee9e14251`, WP-7 `5b6a01837d2836a33a000489ff6dad4519ca40ca67e48464cc384b84721c8195`, WP-7.5 `0fd213f5ace8e077790d81a1c49b78a3fff3f1912a01aef5b52b7df6d1460240`; live category maximum 96 under contract 500; canonical `sha256:2e3571...`; final pointer `2568.0.0`/710, zero drafts, flags false, BOQ 198/1,547, Factor F `2569.0.0`/36 | Exact source, incremental smoke, clean `017`-`025`, and P-42 recovery preflight passed; Spot-check 1 is retained and only Spots 2-4 plus cleanup remain open |
| P-42 review snapshot/terminal-state correction and recovery | Draft review URL identity, stale hard stop, terminal read-only wording, preserved return context, DB-guard authority, incident evidence, recovery baseline, functional Cards A-G, compact-overview containment, and cleanup agree | Incident source `d00c941`; correction `b2500b5e...`; recovery source `f8c6709...`; bounded findings `bdc104f...`; Owner-accepted responsive correction `bcc0417...`; session `session-p42-scored-20260719-f8c6709.json`; inputs 708/708/693; authority 710/65/17; Card F request `35defa1c-...` exactly one effect; responsive checks at 1,443 px and 390 x 844; clean pointer `2568.0.0`/710, zero drafts, flags false; Notes #35/#36/#38 | Source correction, separately approved recovery, functional A-G, cleanup, P42-UAT-OV01, and Spot-check 1 passed; completed Card B-E evidence is retained and Spots 2-4 plus cleanup remain HOLD |
| P-42 proportional spot-check correction | The visible import progress label, real guarded persistence action, written E-01 observation, and tracked cleanup session shape agree without changing DB authority | Spot-check 1 passed on exact `fd36be2`. Spot-check 2 returned `IMPORT_PRICE_AUTHORITY_REQUIRED` with no persistence and exposed P42-UAT-C03/G01. D003 was audited-abandoned; legacy cleanup restored flags before refusing evidence closure at one versus two versions. Readback: pointer `2568.0.0`/710, zero drafts, flags false, BOQ 198/1,547, Factor F `2569.0.0`/36, Production untouched. Exact implementation `44f54a72b03549de995b431d6705ec1b2eeb3fa6` passed focused 2 files/22 tests, full 36 files/230 tests, TypeScript, lint 0 errors/10 existing warnings, authority 710/65/17, script syntax, network-enabled production build, and diff check | Source correction passed; Spot-check 1 retained; Spots 2-4 and schema-2 `bounded-spot-check` cleanup remain HOLD |
| Standard DB lint + temp-aware placement lint | Genuine PL/pgSQL findings fail; the known analyzer inability to resolve a function-created `pg_temp` table is not mistaken for runtime evidence | Standard CLI repeated the documented `pg_temp.catalog_placement_input` false positive and unused `v_row_count` warning. Transaction-scoped temp-aware `plpgsql_check` created the exact two temp schemas and returned zero findings; full placement rollback/race/replay/runtime tests passed. Changing accepted `021` merely to silence the generic analyzer is deferred to bounded P-12 tooling/debt review. | Accepted known limitation; no runtime blocker |
| Security advisor | No new or untriaged blocker | P39R-C CLI security advisor returned no issue. The prior Studio authenticated-callable function warnings remain triaged for P-12 least-privilege review; all are anon-denied. | Passed P39R-C technical scope; P-12 minimization review remains |
| Performance advisor | No rollout blocker | P39R-C CLI advisor returned zero errors: 16 RLS init-plan warnings, 2 multiple-permissive-policy warnings, 7 unindexed-FK information findings, and 38 fresh-reset unused-index information findings; both `020` authority FKs have covering indexes. | Passed P39R-C technical scope; baseline disposition remains due at P-12 |
...
|---|---|---|---|
| 8 authenticated-callable `SECURITY DEFINER` warnings | All deny anon. Seven are baseline RPCs; `get_catalog_publish_readiness` is the guarded Phase 4 facade. Reassess necessity/grants and minimize baseline `get_user_role`/`is_admin` before Production readiness. | Developer + security reviewer + owner | Before P-12 |
| 19 auth RLS init-plan and 5 multiple permissive policy warnings on baseline tables | Pre-existing and outside candidate `020`; no new authority table appeared. Reassess query/policy performance and disposition before Production readiness. | Developer + owner | Before P-12 |
| 7 baseline unindexed-FK information findings | All are pre-existing baseline relations. The two `020` frozen-authority FKs have valid covering indexes. Review baseline index value against write/read workload before Production readiness. | Developer + database reviewer | Before P-12 |
| `private.catalog_placement_state.v_row_count` assigned but never read | Genuine low-risk code-quality debt with no behavior effect. Do not rewrite accepted Local migration `021`; remove through a reviewed forward migration/source checkpoint and rerun affected fingerprints/harnesses. | Developer + database reviewer | Before P-12 |
...
UAT and cleanup on `f36d896d672609653de6634e307dcc44bce6d519`. P-37 remains
HOLD for the final owner UI submission and the other open WP-8 evidence rows in
[Closure Matrix #34](./34-phase4-wp8-p37-closure-matrix.md); Review Note #33
remains the detailed authority for the completed placement UX checks.
