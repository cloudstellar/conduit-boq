...
**Current P-12 readiness disposition (2026-07-25):** desk review and Local
read-only verification are complete on exact implementation checkpoint
...
is HOLD pending separately authorized fresh Production baseline/ledger/advisor
evidence, backup/isolated-restore proof, executor details, managed-residual
acceptance, and the exact window decision. P-12 is not requested.
...
| Advisors | No unresolved Phase 4 blocker | Package #39 records zero Local security issues and the latest performance baseline: 16 init-plan, 2 multiple-permissive-policy, 7 unindexed-FK, and 38 unused-index information findings; both authority FKs are indexed. The accepted `021` dead assignment is recommended as a managed residual instead of a warning-only `026`. | Local disposition complete; fresh Production advisors and Owner residual decision pending |
...
| Security advisor | No new or untriaged blocker | P39R-C CLI security advisor returned no issue. Fresh Local ACL readback found all 13 public definers anon-denied, 8 authenticated-callable facades/baseline RPCs, RLS on all Phase 4 tables, and no authenticated direct writes. `private` is not exposed in the Local Data API. | Local disposition passed; Production exposed-schema/grant/advisor check pending |
| Performance advisor | No rollout blocker | Latest accepted Local CLI advisor returned zero errors: 16 RLS init-plan warnings, 2 multiple-permissive-policy warnings, 7 unindexed-FK information findings, and 38 fresh-reset unused-index information findings; both `020` authority FKs have covering indexes. | Local disposition passed; fresh Production advisor pending |
| CI exact commit | Passed | Exact `6827ebc1a729b7675fe91db58e129c9381b33ddb` passed local repository/build gates; remote CI must match the eventual reviewed head | Local passed; remote exact-head CI pending before P-12 |
...
|---|---|---|---|
| 8 authenticated-callable public `SECURITY DEFINER` functions | Fresh Local readback confirms all deny anon, direct table writes are denied, and only `public`/`graphql_public` are API-exposed. Keep the guarded facades/baseline RPCs; verify the actual Production exposed schemas and grants before P-12. | Developer + security reviewer + owner | Fresh Production evidence and Owner decision at P-12 |
| 16 auth RLS init-plan and 2 multiple permissive policy warnings on baseline tables | Latest accepted Local baseline; no Phase 4 blocker or new authority-table finding. Reassess against fresh Production workload/advisor output. | Developer + owner | Fresh Production evidence and Owner decision at P-12 |
| 7 baseline unindexed-FK information findings | Pre-existing baseline relations; both `020` frozen-authority FKs have valid covering indexes. Add no speculative index without measured workload value. | Developer + database reviewer | Fresh Production evidence and Owner decision at P-12 |
