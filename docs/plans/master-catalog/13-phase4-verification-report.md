...
is pushed. A final fresh no-reset session passed leave/return/reload recovery,
gap-list `ArrowDown`/`Enter`, final review, reset, and cleanup; pushed checkpoint
`96c2ac6892e8ffe9d020c2dff641a847157cd4b2` corrected its stale
recovery-banner finding. Complete native-button keyboard traversal and the P-37
owner decision remain open. WP-8 is In progress; P-37 remains HOLD and Production
...
| 4C clean rehearsal | Local | Codex + owner/developer | 2026-07-15 |  | P-36 integrated technical rehearsal, P-37 technical continuation, and final leave/reload recovery passed; WP-8 remains In progress and P-37 remains HOLD | Exact checkout `910cc3c` passed the integrated chain through `021`, DB/RLS/concurrency/P-20/WP-7/WP-7.5, export/advisors/repository gates, 710 inherited plus 18-new-item route-render measurements, and disabled-baseline cleanup. The first 2026-07-17 intended-admin session failed comprehension; the bounded UI correction passed working-tree desktop/mobile interaction QA. The first 2026-07-18 no-reset continuation passed stale rejection, one accepted placement, exact replay, accepted-state route readback, cleanup, and post-evidence repository checks. A final fresh no-reset session passed leave/return/reload recovery, gap-list `ArrowDown`/`Enter`, final review, reset, and cleanup; checkpoint `96c2ac6892e8ffe9d020c2dff641a847157cd4b2` fixed and re-verified its stale recovery-alert finding. Complete native-button keyboard traversal remains. |
...
| P-37 WP-8 accept/hold | Owner | HOLD retained after failed comprehension gate; later technical continuation and leave/reload recovery do not infer acceptance | 2026-07-17/18 | The first session confirmed no placement. The insertion-gap correction preserves the DB contract. The later Local continuation passed stale rejection, one confirmation, same-ID replay, accepted-state readback, cleanup, and post-evidence repository checks. Final fresh no-reset UAT passed leave/return/reload recovery, gap-list `ArrowDown`/`Enter`, final review, reset, and cleanup; checkpoint `96c2ac6892e8ffe9d020c2dff641a847157cd4b2` fixed the stale recovery alert. Complete native-button keyboard traversal and the explicit owner decision remain. Add/Supplement stays hidden. See [P-37 UAT/UX Note](./33-phase4-wp8-p37-uat-ux-correction-note.md). |
...
The final fresh no-reset P-37 session then passed insertion-gap search with
`ArrowDown`/`Enter`, leave/return/reload recovery, final review, reset, and
cleanup without a placement submission. It found a stale recovery alert after
reset-all; checkpoint `96c2ac6892e8ffe9d020c2dff641a847157cd4b2`
corrected that state and re-UAT passed. Complete native-button keyboard
traversal and explicit owner acceptance are now the remaining P-37 release
gates; Add/Supplement remains hidden.
...
is pushed. Final fresh no-reset evidence later passed leave/return/reload
recovery and corrected the stale reset-all recovery alert at pushed checkpoint
`96c2ac6892e8ffe9d020c2dff641a847157cd4b2`. P-37 remains **HOLD** pending
complete native-button keyboard re-UAT and explicit owner acceptance.
...
|---|---|---|---|
| WP-8 clean Local rehearsal | Passed with no unresolved blocker | P-36 integrated technical rehearsal passed; final P-37 no-reset flow/recovery evidence passed and returned Local to the disabled baseline | Pending only complete native-button keyboard UAT and explicit P-37 acceptance |
| WP-6.6 capability matrix | Audit #29 C-01 through C-17 implemented/evidenced, or unsupported controls removed from release visibility | Final G1R/G2/P-25/G3/P-26 path was owner-accepted at `78e96ab`; P-36 integrated DB/repository rerun passed on `910cc3c`; final P-37 recovery evidence passed | Accepted/Complete technically; complete native-button keyboard UAT remains under WP-8 |
...
| End-to-end request idempotency | UI/action/DB reuse one operation ID after timeout; changed payload with same ID rejects | Prior DB/transport/browser recovery passed; P-36 repeated replay/mismatch/race contracts in the integrated harnesses; P-37 repeated stale rejection and exact replay | Passed Local technical/recovery scope; P-37 owner decision remains separate |
...
| Admin UAT and recovery | Intended admin completes core workflow and representative failures without developer/SQL assistance | Corrected 710+18 flow, leave/return/reload recovery, reset, final review, stale handling, and one-batch mechanics passed; the in-app driver could not dispatch native-button traversal | Pending only manual keyboard-only pass and explicit P-37 acceptance |
| 710-row performance baseline | Import preview, readiness, export, and admin interactions meet reviewed budget | P-36 recorded 710+18 route render/deep paging/export measurements; corrected P-37 pointer flow, search, preview, final review, reset, and recovery completed without material stutter, focus loss, or layout failure | Passed bounded Local V1 evidence; owner keyboard perception remains in P-37 |
| Authority/document consistency | Migration/WP order, decision IDs, authority links, and Markdown table shapes agree | P-36 closeout aligned current status and Review Note #32; focused authority consistency passed 1 file/7 tests and the full suite passed 33 files/183 tests; exact closeout `65baf79bd907b31692ff32c5f1083c8cab7a0701` is pushed | Passed P-36 closeout checkpoint |
...
| New-item display order | Mechanical maximum + 1 remains draft allocation only; publish requires a current accepted WP-7.5 placement review for every identity absent from the base | Amended `021`, P-32/P-33 technical evidence, and P-36 integrated role/race/order/hash/export plus 710+18 route rendering passed. Browser automation did not dispatch client state changes. | Integrated technical gate passed; independent live interaction/UAT remains P-37/P-14 |
...
|---|---|---|---|
| `npm test` | Exit 0 | P-36 exact checkout `910cc3c`: 33 files/183 tests passed. P-37 corrected source checkpoint `e6d79d77bd8fb8d6a0211d7d7b440d2136cb6512`: 33 files/184 tests passed, including insertion-gap boundary mapping. | Passed pushed source checkpoint |
| `npx tsc --noEmit --pretty false` | Exit 0 | P-36 exact checkout and P-37 corrected source checkpoint `e6d79d7` passed. | Passed pushed source checkpoint |
| `npm run lint` | Exit 0 | P-37 corrected source checkpoint `e6d79d7` exited 0 with the same 10 existing application warnings outside this scope. | Passed with existing warnings |
...
| `git diff --check` | Exit 0 | Passed before committing P-37 corrected source checkpoint `e6d79d7` after code/test/document alignment. | Passed pushed source checkpoint |
...
therefore remains HOLD for complete native-button keyboard re-UAT and the
explicit owner decision in
[Review Note #33](./33-phase4-wp8-p37-uat-ux-correction-note.md).
