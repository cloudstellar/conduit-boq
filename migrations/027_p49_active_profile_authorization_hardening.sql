-- =============================================================================
-- P-49 ACTIVE-PROFILE AUTHORIZATION HARDENING
-- =============================================================================
-- Canonical forward-only source prepared under the Owner's bounded P-49
-- pre-deploy authority. This file is not an authorization to apply, commit,
-- push, or deploy. Every mutable operation remains inside one transaction and
-- follows exact predecessor checks plus an advisory lock.
--
...
-- P49 CONTRACT: predecessor-policies-preserved
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- 0. Serialize before any mutable operation
-- -----------------------------------------------------------------------------
...
-- Fresh Production source binding accepted in
-- production-readonly-binding-v2-accepted.json (one SELECT, no retry). Each
-- digest covers every projected field in the accepted query's deterministic
-- manifest order;
-- counts make the check fail closed on missing or additional overloads/rows.
DO $p49_exact_production_binding_preflight$
DECLARE
  v_function_count integer;
  v_function_sha256 text;
  v_trigger_count integer;
  v_trigger_sha256 text;
  v_trigger_relation_count integer;
  v_trigger_manifest_sha256 text;
  v_relation_count integer;
  v_column_acl_sha256 text;
  v_view_count integer;
  v_view_dependency_count integer;
BEGIN
  WITH target_function_names(function_name) AS (
    VALUES
      ...
      ('lock_org_fields_after_onboarding'),
      ('p49_admin_transition'),
      ('p49_current_active_admin'),
      ('p49_current_active_department_id'),
      ('p49_current_active_role'),
      ('p49_current_profile_active'),
      ('p49_current_profile_recognized'),
      ('p49_current_selector_reader'),
      ('p49_guard_user_profile_mutation'),
      ('p49_reject_authorization_event_mutation'),
      ('save_boq_with_routes'),
      ('update_my_profile')
  ), function_posture AS (
    SELECT
      ...
      pg_catalog.has_function_privilege(
        to_regrole('anon')::oid, p.oid, 'EXECUTE'
      ) AS anon_execute,
      pg_catalog.has_function_privilege(
        to_regrole('authenticated')::oid, p.oid, 'EXECUTE'
      ) AS authenticated_execute,
      pg_catalog.has_function_privilege(
        to_regrole('service_role')::oid, p.oid, 'EXECUTE'
    ...
    WHERE n.nspname IN ('public', 'private')
      ...
      AND EXISTS (
        ...
        WHERE target.function_name = p.proname
      )
  )
  SELECT
    count(*)::integer,
          ...
          coalesce(
            pg_catalog.string_agg(
              pg_catalog.concat_ws(
                ...
                result_type, function_kind, language_name, owner_name,
                security_definer::text, volatility, proconfig, acl,
                public_execute::text, anon_execute::text,
                authenticated_execute::text, service_role_execute::text,
                definition_sha256
              ),
  ...
  INTO v_function_count, v_function_sha256
  FROM function_posture;

  IF v_function_count <> 9
     OR v_function_sha256 IS DISTINCT FROM
       '78b94944d72e761a0e473709f7a9cce3223c8dacb26b5f84cad0bf1d5450e88d' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 preflight blocked: exact Production function posture drifted',
      DETAIL = pg_catalog.format(
        'rows=%s sha256=%s', v_function_count, v_function_sha256
      );
  END IF;
  ...
  WITH target_relations(schema_name, relation_name) AS (
    VALUES
      ...
      ('public', 'user_authorization_events'),
      ('public', 'app_settings'),
    ...
    SELECT
      ...
      fn.prosecdef AS function_security_definer,
      coalesce(fn.proconfig::text, '') AS function_proconfig,
      pg_catalog.encode(
        pg_catalog.sha256(
          pg_catalog.convert_to(pg_catalog.pg_get_functiondef(fn.oid), 'UTF8')
        ),
        'hex'
    ...
    WHERE NOT t.tgisinternal
      AND EXISTS (
        ...
        WHERE target.schema_name = n.nspname
          AND target.relation_name = c.relname
      )
  )
  SELECT
    count(*)::integer,
      ...
      pg_catalog.sha256(
        pg_catalog.convert_to(
          coalesce(
            pg_catalog.string_agg(
              pg_catalog.concat_ws(
                '|', schema_name, relation_name, trigger_name,
                is_internal::text, enabled_mode, function_name,
                function_identity_arguments, function_owner,
                function_security_definer::text, function_proconfig,
                function_definition_sha256, definition_sha256
              ),
  ...
  INTO v_trigger_count, v_trigger_sha256
  FROM trigger_posture;

  IF v_trigger_count <> 11
     OR v_trigger_sha256 IS DISTINCT FROM
       'aff1c9ac1a5a09cf5d67dcc23be0e29be1ba8c251ed4c84969ce429bf24e9066' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 preflight blocked: exact Production trigger posture drifted',
      DETAIL = pg_catalog.format(
        'rows=%s sha256=%s', v_trigger_count, v_trigger_sha256
      );
  END IF;
  ...
  WITH target_relations(schema_name, relation_name) AS (
    ...
    SELECT
      ...
      count(t.oid)::integer AS trigger_count,
      pg_catalog.encode(
        pg_catalog.sha256(
          pg_catalog.convert_to(
            coalesce(
              pg_catalog.string_agg(
                pg_catalog.concat_ws(
                  '|', t.tgname, t.tgisinternal::text, t.tgenabled::text,
                  pg_catalog.pg_get_triggerdef(t.oid, true),
                  pg_catalog.pg_get_function_identity_arguments(fn.oid),
                  pg_catalog.pg_get_functiondef(fn.oid)
                ),
    ...
    LEFT JOIN pg_catalog.pg_proc fn ON fn.oid = t.tgfoid
    GROUP BY target.schema_name, target.relation_name
  )
  SELECT
    count(*)::integer,
    pg_catalog.encode(
      pg_catalog.sha256(
        pg_catalog.convert_to(
          coalesce(
            pg_catalog.string_agg(
              pg_catalog.concat_ws(
                '|', schema_name, relation_name, trigger_count::text, sha256
              ),
  ...
  INTO v_trigger_relation_count, v_trigger_manifest_sha256
  FROM trigger_manifest;

  IF v_trigger_relation_count <> 14
     OR v_trigger_manifest_sha256 IS DISTINCT FROM
       '14d9df8f0415e5c3114d21d1204678f76c3ed694360a0cf1d671e1768f52d900' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 preflight blocked: exact Production trigger inventory drifted',
      DETAIL = pg_catalog.format(
        'relations=%s sha256=%s',
        v_trigger_relation_count,
        v_trigger_manifest_sha256
      );
  END IF;
  ...
  WITH target_relations(schema_name, relation_name) AS (
    ...
    SELECT
      ...
      ) END AS explicit_column_acl_count,
      CASE WHEN relation.oid IS NULL THEN NULL ELSE pg_catalog.encode(
        pg_catalog.sha256(
          pg_catalog.convert_to(
            coalesce((
                ...
                pg_catalog.jsonb_build_object(
                  ...
                  'anon', pg_catalog.jsonb_build_object(
                    'select', pg_catalog.has_column_privilege(
                      to_regrole('anon')::oid, relation.oid, a.attnum, 'SELECT'
                    ),
                    'insert', pg_catalog.has_column_privilege(
                      to_regrole('anon')::oid, relation.oid, a.attnum, 'INSERT'
                    ),
                    'update', pg_catalog.has_column_privilege(
                      to_regrole('anon')::oid, relation.oid, a.attnum, 'UPDATE'
                    ),
                    'references', pg_catalog.has_column_privilege(
                      to_regrole('anon')::oid, relation.oid, a.attnum,
                      'REFERENCES'
                    )
                  ),
                  'authenticated', pg_catalog.jsonb_build_object(
                    'select', pg_catalog.has_column_privilege(
                      to_regrole('authenticated')::oid, relation.oid, a.attnum,
                      'SELECT'
                    ),
                    'insert', pg_catalog.has_column_privilege(
                      to_regrole('authenticated')::oid, relation.oid, a.attnum,
                  ...
                  'service_role', pg_catalog.jsonb_build_object(
                    'select', pg_catalog.has_column_privilege(
                      to_regrole('service_role')::oid, relation.oid, a.attnum,
                      'SELECT'
                    ),
                    'insert', pg_catalog.has_column_privilege(
                      to_regrole('service_role')::oid, relation.oid, a.attnum,
        ...
        'hex'
      ) END AS column_acl_manifest_sha256
    FROM target_relations target
      ...
      pg_catalog.sha256(
        pg_catalog.convert_to(
          coalesce(
            pg_catalog.string_agg(
              pg_catalog.concat_ws(
                '|', schema_name, relation_name, relation_exists::text,
                relation_kind, explicit_column_acl_count::text,
                column_acl_manifest_sha256
              ),
              E'\n' ORDER BY schema_name, relation_name
            ),
            ''
  ...
  INTO v_relation_count, v_column_acl_sha256
  FROM relation_column_acl;

  IF v_relation_count <> 13
     OR v_column_acl_sha256 IS DISTINCT FROM
       '035fc92695de7bb1d2b2f9ef66396cfdc0324b3f76f9e572bfc514bb3ca7d323'
     OR to_regclass('public.user_authorization_events') IS NOT NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 preflight blocked: exact Production column ACL posture drifted',
      DETAIL = pg_catalog.format(
        'relations=%s sha256=%s audit_relation=%s',
        v_relation_count,
        v_column_acl_sha256,
        to_regclass('public.user_authorization_events')
      );
  END IF;
  ...
  WITH target_relations(schema_name, relation_name) AS (
    ...
    VALUES
      ('admin_approve_user'), ('admin_deactivate_user'),
      ('admin_reactivate_user'), ('admin_reject_user'),
      ('admin_set_user_role'), ('admin_suspend_user'), ('can_approve_boq'),
      ('get_admin_profiles_page'), ('get_authorized_profiles_page'),
      ('get_my_catalog_capabilities'), ('get_my_profile'),
      ('get_my_profile_v2'), ('get_user_role'), ('handle_new_user'),
      ('is_admin'), ('is_signup_email_allowed'),
      ('lock_org_fields_after_onboarding'), ('p49_admin_transition'),
      ('p49_current_active_admin'), ('p49_current_active_department_id'),
      ('p49_current_active_role'), ('p49_current_profile_active'),
      ('p49_current_profile_recognized'), ('p49_current_selector_reader'),
      ('p49_guard_user_profile_mutation'),
      ('p49_reject_authorization_event_mutation'),
      ('save_boq_with_routes'), ('update_my_profile')
  ), relevant_view_dependencies AS (
    SELECT dependency.objid, dependency.refobjid
    FROM pg_catalog.pg_class view_relation
    JOIN pg_catalog.pg_namespace view_namespace
      ON view_namespace.oid = view_relation.relnamespace
    JOIN pg_catalog.pg_rewrite rewrite_row
      ON rewrite_row.ev_class = view_relation.oid
    ...
    WHERE view_namespace.nspname IN ('public', 'private')
      AND view_relation.relkind IN ('v', 'm')
      AND EXISTS (
        SELECT 1 FROM target_relations target
        WHERE target.schema_name = target_namespace.nspname
          AND target.relation_name = target_relation.relname
      )
    UNION
    SELECT dependency.objid, dependency.refobjid
    FROM pg_catalog.pg_class view_relation
    ...
    JOIN pg_catalog.pg_proc target_function
      ON target_function.oid = dependency.refobjid
    JOIN pg_catalog.pg_namespace target_namespace
    ...
    WHERE view_namespace.nspname IN ('public', 'private')
      ...
      AND EXISTS (
        SELECT 1 FROM target_function_names target
        WHERE target.function_name = target_function.proname
      )
  )
  SELECT count(*)::integer
  INTO v_view_dependency_count
  FROM relevant_view_dependencies;

  IF v_view_count <> 0 OR v_view_dependency_count <> 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 preflight blocked: Production view posture drifted',
      DETAIL = pg_catalog.format(
        'views=%s relevant_dependencies=%s',
        v_view_count,
        v_view_dependency_count
      );
  END IF;
END;
$p49_exact_production_binding_preflight$;
...
-- Bind the exact 026 policy and effective relation-ACL predecessor. These
-- digests were reproduced on the isolated PostgreSQL 17 baseline. A Production
-- apply must recompute them in the target transaction and fails before DDL on
-- any drift.
DO $p49_exact_policy_acl_preflight$
DECLARE
  v_policy_count integer;
  v_policy_sha256 text;
  v_acl_count integer;
  v_acl_sha256 text;
BEGIN
  WITH target_tables(table_name) AS (VALUES
    ('app_settings'), ('organizations'), ('departments'), ('sectors'),
    ('boq'), ('boq_items'), ('boq_routes'), ('factor_reference'),
    ('factor_reference_versions'), ('factor_reference_rows'),
    ('factor_reference_default_version'), ('user_profiles')
  ), policy_manifest AS (
    SELECT p.*
  ...
  INTO v_policy_count, v_policy_sha256
  FROM policy_manifest;
  ...
  WITH target_tables(table_name) AS (VALUES
    ...
    LEFT JOIN pg_catalog.pg_roles r ON r.oid = privilege.grantee
          ...
          coalesce(
            pg_catalog.string_agg(
              pg_catalog.jsonb_build_object(
                ...
                'table', relname,
                'rls', relrowsecurity,
                'forceRls', relforcerowsecurity,
                'owner', owner_name,
                'grantee', grantee_name,
                'privilege', privilege_type,
                'grantable', is_grantable
              )::text,
              E'\n' ORDER BY nspname, relname, grantee_name,
  ...
  INTO v_acl_count, v_acl_sha256
  FROM acl_manifest;

  IF v_policy_count <> 27
     OR v_policy_sha256 IS DISTINCT FROM
       '937f49eab393874d93981763db569f1588a69fa9b9dfad3f6973eb63f48b301e'
     OR v_acl_count <> 267
     OR v_acl_sha256 IS DISTINCT FROM
       'd9198543971d1e6386109cb148d5bb1085f6628ef7691f345a30d746c1445fe2' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 preflight blocked: exact predecessor policy/ACL manifest drifted',
      DETAIL = pg_catalog.format(
        'policy=%s/%s acl=%s/%s',
        v_policy_count, v_policy_sha256, v_acl_count, v_acl_sha256
      );
  END IF;
END;
$p49_exact_policy_acl_preflight$;

DO $p49_new_object_collision_preflight$
DECLARE
  v_signature text;
BEGIN
  FOREACH v_signature IN ARRAY ARRAY[
    'private.p49_current_profile_recognized()',
    'private.p49_current_profile_active()',
    'private.p49_current_active_role()',
    'private.p49_current_active_department_id()',
    'private.p49_current_selector_reader()',
    'private.p49_current_active_admin()',
    'private.p49_reject_authorization_event_mutation()',
    'private.p49_guard_user_profile_mutation()',
    'private.p49_admin_transition(uuid,text,text,text,uuid)',
    'public.get_my_profile_v2()',
    'public.update_my_profile(text,text,text,text,text,text,uuid,uuid,boolean,uuid)',
    'public.get_authorized_profiles_page(integer,timestamptz,uuid)',
    'public.get_admin_profiles_page(integer,timestamptz,uuid)',
    'public.admin_approve_user(uuid,uuid,text)',
    'public.admin_reject_user(uuid,text,uuid)',
    'public.admin_suspend_user(uuid,text,uuid)',
    'public.admin_deactivate_user(uuid,text,uuid)',
    'public.admin_reactivate_user(uuid,text,uuid)',
    'public.admin_set_user_role(uuid,text,text,uuid)',
  ...
  LOOP
    IF to_regprocedure(v_signature) IS NOT NULL THEN
      RAISE EXCEPTION
        'P49 preflight blocked: new function signature already exists: %',
        v_signature;
    END IF;
  END LOOP;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'P49 preflight blocked: new profile guard trigger already exists';
  END IF;
END;
$p49_new_object_collision_preflight$;
...
-- Preserve every trigger that this migration does not intentionally replace.
-- The exact Production trigger manifests above bind this capture before any
-- permanent-object DDL; the final postimage compares the rows bidirectionally.
CREATE TEMP TABLE p49_unchanged_trigger_preimage
ON COMMIT DROP
AS
WITH target_relations(schema_name, relation_name) AS (
  VALUES
...
SELECT
  ...
  pg_catalog.pg_get_function_identity_arguments(fn.oid)
    AS function_identity_arguments,
  pg_catalog.pg_get_function_result(fn.oid) AS function_result,
  fn.prokind::text AS function_kind,
  fn_language.lanname AS function_language,
  pg_catalog.pg_get_userbyid(fn.proowner) AS function_owner,
  fn.prosecdef AS function_security_definer,
  fn.provolatile::text AS function_volatility,
  coalesce(fn.proconfig::text, '') AS function_proconfig,
  coalesce(fn.proacl::text, '') AS function_acl,
  EXISTS (
    SELECT 1
    FROM pg_catalog.aclexplode(
  ...
  pg_catalog.has_function_privilege(
    to_regrole('anon')::oid, fn.oid, 'EXECUTE'
  ) AS function_anon_execute,
  pg_catalog.has_function_privilege(
    to_regrole('authenticated')::oid, fn.oid, 'EXECUTE'
  ) AS function_authenticated_execute,
  pg_catalog.has_function_privilege(
    to_regrole('service_role')::oid, fn.oid, 'EXECUTE'
  ) AS function_service_role_execute,
  pg_catalog.pg_get_functiondef(fn.oid) AS function_definition
FROM pg_catalog.pg_trigger t
JOIN pg_catalog.pg_class c ON c.oid = t.tgrelid
JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace
JOIN pg_catalog.pg_proc fn ON fn.oid = t.tgfoid
JOIN pg_catalog.pg_namespace fn_namespace
  ON fn_namespace.oid = fn.pronamespace
JOIN pg_catalog.pg_language fn_language ON fn_language.oid = fn.prolang
WHERE EXISTS (
    SELECT 1
    FROM target_relations target
    WHERE target.schema_name = n.nspname
...
-- retain owner/service-role access; callers use the two bounded projections.
...
-- Preserve each existing permissive selector policy and add one restrictive
-- current-profile boundary. Active admins retain their predecessor row scope;
-- pending/active non-admin callers see only an active, valid hierarchy.
CREATE POLICY p49_organizations_recognized_active_hierarchy
ON public.organizations AS RESTRICTIVE
FOR SELECT TO authenticated
USING (
  (SELECT private.p49_current_active_admin())
  OR (
    (SELECT private.p49_current_selector_reader())
    AND COALESCE(is_active, false)
  )
);

CREATE POLICY p49_departments_recognized_active_hierarchy
ON public.departments AS RESTRICTIVE
FOR SELECT TO authenticated
USING (
  (SELECT private.p49_current_active_admin())
  OR (
    ...
    AND EXISTS (
      ...
      WHERE o.id = departments.org_id
        AND COALESCE(o.is_active, false)
    )
  )
);

CREATE POLICY p49_sectors_recognized_active_hierarchy
ON public.sectors AS RESTRICTIVE
FOR SELECT TO authenticated
USING (
  (SELECT private.p49_current_active_admin())
  OR (
    ...
    AND EXISTS (
      ...
      FROM public.departments d
      JOIN public.organizations o ON o.id = d.org_id
      WHERE d.id = sectors.department_id
        AND COALESCE(d.is_active, false)
        AND COALESCE(o.is_active, false)
    )
  )
...
-- -----------------------------------------------------------------------------
-- 5. BOQ header/child current-active boundary
-- -----------------------------------------------------------------------------
-- Preserve every predecessor owner/assignee/department/sector/status branch.
-- A restrictive policy composes with those permissive policies and adds one
-- current-active requirement for every BOQ command without copying their
-- expressions into this migration.
...
CREATE POLICY p49_boq_items_current_active
ON public.boq_items AS RESTRICTIVE
FOR ALL TO authenticated
USING ((SELECT private.p49_current_profile_active()))
WITH CHECK ((SELECT private.p49_current_profile_active()));

CREATE POLICY p49_boq_routes_current_active
ON public.boq_routes AS RESTRICTIVE
FOR ALL TO authenticated
USING ((SELECT private.p49_current_profile_active()))
WITH CHECK ((SELECT private.p49_current_profile_active()));
...
-- -----------------------------------------------------------------------------
-- -----------------------------------------------------------------------------
-- 6. Legacy and versioned Factor F current-active boundary
-- -----------------------------------------------------------------------------
-- Preserve all predecessor default/version/status/admin predicates and compose
-- a restrictive active-profile gate with them.
CREATE POLICY p49_factor_reference_current_active
ON public.factor_reference AS RESTRICTIVE
FOR ALL TO authenticated
USING ((SELECT private.p49_current_profile_active()))
WITH CHECK ((SELECT private.p49_current_profile_active()));

CREATE POLICY p49_factor_reference_versions_current_active
ON public.factor_reference_versions AS RESTRICTIVE
FOR SELECT TO authenticated
USING ((SELECT private.p49_current_profile_active()));

CREATE POLICY p49_factor_reference_rows_current_active
ON public.factor_reference_rows AS RESTRICTIVE
FOR SELECT TO authenticated
USING ((SELECT private.p49_current_profile_active()));

CREATE POLICY p49_factor_reference_default_version_current_active
ON public.factor_reference_default_version AS RESTRICTIVE
FOR SELECT TO authenticated
USING ((SELECT private.p49_current_profile_active()));
...
-- -----------------------------------------------------------------------------
-- 7. Exact policy and relation-ACL postconditions
-- -----------------------------------------------------------------------------
DO $p49_policy_acl_postconditions$
DECLARE
  v_policy_count integer;
...
BEGIN
  SELECT
    ...
    count(*) FILTER (
      WHERE p.permissive = 'RESTRICTIVE'
        AND p.policyname IN (
          'p49_organizations_recognized_active_hierarchy',
          'p49_departments_recognized_active_hierarchy',
          'p49_sectors_recognized_active_hierarchy',
          'p49_boq_current_active',
          'p49_boq_items_current_active',
          'p49_boq_routes_current_active',
          'p49_factor_reference_current_active',
          'p49_factor_reference_versions_current_active',
          'p49_factor_reference_rows_current_active',
          'p49_factor_reference_default_version_current_active'
        )
        AND p.roles = ARRAY['authenticated'::name]
        AND p.qual ILIKE '%p49_current_%'
    )::integer
  INTO v_policy_count, v_restrictive_count
  FROM pg_catalog.pg_policies p
  WHERE p.schemaname = 'public'
    AND p.tablename IN (
      'app_settings', 'organizations', 'departments', 'sectors',
      ...
      'factor_reference_versions', 'factor_reference_rows',
      'factor_reference_default_version', 'user_profiles',
      'user_authorization_events'
    );

  IF v_policy_count <> 32 OR v_restrictive_count <> 10 THEN
    RAISE EXCEPTION
      'P49 postcondition failed: policy inventory is % rows/% restrictive rows',
      v_policy_count,
      v_restrictive_count;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'P49 postcondition failed: raw app_settings policy remains';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'P49 postcondition failed: a protected relation has RLS disabled';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'P49 postcondition failed: anon retains a protected relation privilege';
  END IF;

  IF pg_catalog.has_table_privilege('authenticated', 'public.app_settings', 'SELECT')
     OR pg_catalog.has_table_privilege('authenticated', 'public.app_settings', 'INSERT')
     OR pg_catalog.has_table_privilege('authenticated', 'public.app_settings', 'UPDATE')
     OR pg_catalog.has_table_privilege('authenticated', 'public.app_settings', 'DELETE')
     OR NOT pg_catalog.has_table_privilege('authenticated', 'public.user_profiles', 'SELECT')
     OR pg_catalog.has_table_privilege('authenticated', 'public.user_profiles', 'INSERT')
     OR pg_catalog.has_table_privilege('authenticated', 'public.user_profiles', 'UPDATE')
     OR pg_catalog.has_table_privilege('authenticated', 'public.user_profiles', 'DELETE')
     OR NOT pg_catalog.has_table_privilege('authenticated', 'public.user_authorization_events', 'SELECT')
     OR pg_catalog.has_table_privilege('authenticated', 'public.user_authorization_events', 'INSERT')
     OR pg_catalog.has_table_privilege('authenticated', 'public.user_authorization_events', 'UPDATE')
     OR pg_catalog.has_table_privilege('authenticated', 'public.user_authorization_events', 'DELETE') THEN
    RAISE EXCEPTION
      'P49 postcondition failed: bounded profile/settings/audit ACL mismatch';
  END IF;

  IF EXISTS (
    SELECT 1
    FROM (VALUES
      ('organizations'), ('departments'), ('sectors'),
      ('factor_reference_versions'), ('factor_reference_rows'),
    ...
    RAISE EXCEPTION
      'P49 postcondition failed: selector/versioned Factor F ACL mismatch';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'P49 postcondition failed: authenticated business-table ACL mismatch';
  END IF;
END;
$p49_policy_acl_postconditions$;
...
DO $p49_exact_postimage_postcondition$
DECLARE
  v_function_count integer;
  v_function_sha256 text;
  v_trigger_count integer;
...
BEGIN
  ...
  IF v_function_count <> 27
     OR v_function_sha256 IS DISTINCT FROM
       '582353bcd34980a0a267bb2a1ccfac36bec7cbb70633698bbed428e2e9e17621' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 postcondition failed: exact function postimage drifted',
      DETAIL = pg_catalog.format(
        'rows=%s sha256=%s', v_function_count, v_function_sha256
      );
  END IF;
  ...
  IF v_trigger_count <> 12
     OR v_trigger_sha256 IS DISTINCT FROM
       '6bc323aa75d604c158022c4ec17166fa210717a63a4682edeeb5ee48ea701c5a' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 postcondition failed: exact trigger postimage drifted',
      DETAIL = pg_catalog.format(
        'rows=%s sha256=%s', v_trigger_count, v_trigger_sha256
      );
  END IF;
  ...
  WITH target_tables(table_name) AS (
    ...
    SELECT p.*
    FROM pg_catalog.pg_policies p
    JOIN target_tables target ON target.table_name = p.tablename
    WHERE p.schemaname = 'public'
  )
  SELECT
    count(*)::integer,
  ...
  IF v_policy_count <> 32
     OR v_policy_sha256 IS DISTINCT FROM
       '00ad043e39dfc54178524691c89117bdb2ca5091f08bf3dc77f4217e777db6c1' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 postcondition failed: exact policy postimage drifted',
      DETAIL = pg_catalog.format(
        'rows=%s sha256=%s', v_policy_count, v_policy_sha256
      );
  END IF;
  ...
  WITH target_tables(table_name) AS (
    ...
    FROM pg_catalog.pg_class c
    JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace
    JOIN target_tables target ON target.table_name = c.relname
    CROSS JOIN LATERAL pg_catalog.aclexplode(
      coalesce(c.relacl, pg_catalog.acldefault('r', c.relowner))
    ) privilege
    LEFT JOIN pg_catalog.pg_roles r ON r.oid = privilege.grantee
  ...
  IF v_acl_count <> 216
     OR v_acl_sha256 IS DISTINCT FROM
       '35e3c4fc068c87a89617dd2e5ac114c7fedbe64eaf7e9d5e3482c4cda4312da6' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 postcondition failed: exact relation ACL postimage drifted',
      DETAIL = pg_catalog.format(
        'rows=%s sha256=%s', v_acl_count, v_acl_sha256
      );
  END IF;
  ...
  IF v_relation_count <> 14
     OR v_column_acl_sha256 IS DISTINCT FROM
       '1a59fe2497f7fe371dfa848a0543a32be00b884d19a6dd4a4997568892325e2b' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 postcondition failed: exact column ACL postimage drifted',
      DETAIL = pg_catalog.format(
        'relations=%s sha256=%s', v_relation_count, v_column_acl_sha256
      );
  END IF;

  SELECT
    count(*)::integer,
    min(pg_catalog.encode(
      pg_catalog.sha256(
        pg_catalog.convert_to(pg_catalog.pg_get_triggerdef(t.oid, true), 'UTF8')
      ),
      'hex'
    ))
  INTO v_auth_trigger_count, v_auth_trigger_sha256
  FROM pg_catalog.pg_trigger t
  WHERE t.tgrelid = 'auth.users'::regclass
    AND t.tgname = 'on_auth_user_created'
    AND NOT t.tgisinternal;

  IF v_auth_trigger_count <> 1
     OR v_auth_trigger_sha256 IS DISTINCT FROM
       '0da996fbca2881e1359d414ec2e0123466a0e9223b7157e83b0c5fc0cf3d1500' THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 postcondition failed: Auth trigger definition changed',
      DETAIL = pg_catalog.format(
        'rows=%s sha256=%s', v_auth_trigger_count, v_auth_trigger_sha256
      );
  END IF;
  ...
  IF EXISTS (
    WITH target_relations(schema_name, relation_name) AS (
      VALUES
        ...
        ('public', 'factor_reference_rows'),
        ('public', 'factor_reference_default_version')
    ), current_unchanged_trigger_postimage AS (
      SELECT
        n.nspname AS schema_name,
        c.relname AS relation_name,
        t.tgname AS trigger_name,
      ...
      WHERE EXISTS (
        ...
        AND NOT (
          n.nspname = 'public'
          AND c.relname = 'user_profiles'
          AND t.tgname IN ('trg_lock_org', 'trg_p49_guard_user_profile_mutation')
        )
        AND NOT (
          n.nspname = 'public'
          AND c.relname = 'user_authorization_events'
          AND t.tgname = 'trg_p49_authorization_events_immutable'
        )
    ), trigger_drift AS (
      (
        SELECT * FROM p49_unchanged_trigger_preimage
        EXCEPT ALL
        SELECT * FROM current_unchanged_trigger_postimage
      )
      UNION ALL
      (
        SELECT * FROM current_unchanged_trigger_postimage
    ...
    SELECT 1 FROM trigger_drift
  ) THEN
    RAISE EXCEPTION
      'P49 postcondition failed: an unchanged trigger/function binding drifted';
  END IF;
  ...
  IF v_view_count <> 0 OR v_view_dependency_count <> 0 THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'P49 postcondition failed: view postimage drifted',
      DETAIL = pg_catalog.format(
        'views=%s relevant_dependencies=%s',
        v_view_count,
        v_view_dependency_count
  ...
  END IF;
END;
$p49_exact_postimage_postcondition$;
