...
describe('Master Catalog migration contracts', () => {
  ...
  it('keeps the canonical Local bootstrap on the fully rehearsed path', () => {
    ...
    expect(bootstrap).toContain('migrations/017a_master_catalog_phase4_global_function_default_privileges.sql')
    ...
    expect(bootstrap).toContain('psql -v ON_ERROR_STOP=1 -U postgres -d postgres -f /tmp/017a.sql')
    ...
    expect(canonicalHash).toContain("'017, 017a, 018-021'")
    expect(canonicalHash).toContain('phase4_global_function_default_acl')
    expect(canonicalHash).toContain('migration017aDetected')
    expect(canonicalHash).toContain("namespace.nspname IN ('public', 'private')")
    expect(canonicalHash).toContain(
      'schema_default.defaclrole = to_regrole(\'postgres\')',
    )
  ...
  it('installs the reviewed 017a global function default-privilege guard before private helpers exist', () => {
    const sql = readMigration(
      '017a_master_catalog_phase4_global_function_default_privileges.sql',
    )

    expect(sql).toContain('Migration 017a: Global function default-privilege guard')
    expect(sql).toContain("SET LOCAL lock_timeout = '10s'")
    expect(sql).toContain("SET LOCAL statement_timeout = '60s'")
    expect(sql).toContain("session_user <> 'postgres' OR current_user <> 'postgres'")
    expect(sql).toContain('expected zero private routines after migration 017')
    ...
    expect(sql).toContain('ALTER DEFAULT PRIVILEGES IN SCHEMA private')
    expect(sql).toContain('REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA private')
    expect(sql).toContain('d.defaclnamespace = 0')
    expect(sql).toContain("d.defaclobjtype = 'f'")
    expect(sql).toContain('v_global_default_entries <> 1')
    ...
    expect(sql).not.toMatch(/catalog_(?:admin|new_identity|retirement)_enabled'\s*,/i)
