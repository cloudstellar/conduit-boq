...
describe('Master Catalog migration contracts', () => {
  ...
  it('keeps the canonical Local bootstrap on the fully rehearsed path', () => {
    ...
    const smoke = readFileSync(resolve(process.cwd(), 'scripts/smoke-master-catalog-phase2.mjs'), 'utf8')
    expect(smoke).toContain('const specialItemName = `${priceItem.item_name} (Steel Pole)`')
    expect(smoke).toContain("assert(savedItem.item_name === specialItemName, 'Saved item lost its BOQ-specific special suffix')")
    expect(smoke).toContain("assert(Number(savedItem.unit_cost) === Number(priceItem.unit_cost), 'Saved item unit cost did not come from the catalog')")
  })
